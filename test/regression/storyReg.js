...
describe('Checking the main functionality', function () {
    ...
    describe('Story Text', function () {
        ...
        it('TC-190 Verify that story/s header is present', function () {
            inputValues4andClick(data.name[1], data.gender.it, data.age[1], data.storyType.Comedy);
            const h3Story = $(sel.headerStory).isDisplayed();
            expect(h3Story).toEqual(true);
        });

        it('TC-191 Verify that story/s header text is correct', function () {
            inputValues4andClick(data.name[1], data.gender.it, data.age[1], data.storyType.Comedy);
            const h3StText = $(sel.headerStory).getText();
            expect(h3StText).toEqual(exp.hStory);
        });

        it('TC-192 Verify that story/s name is present', function () {
            inputValues4andClick(data.name[1], data.gender.it, data.age[1], data.storyType.Comedy);
            const stName = $(sel.storyName).isDisplayed();
            expect(stName).toEqual(true);
        });

        it('TC-193 Verify that story/s name text is correct for Comedy', function () {
            inputValues4andClick(data.name[1], data.gender.it, data.age[1], data.storyType.Comedy);
            const comedyNameText = $(sel.storyName).getText();
            expect(comedyNameText).toEqual(tComedy.comedyName);
        });

        it('TC-194 Verify that story/s context is present', function () {
            inputValues4andClick(data.name[1], data.gender.it, data.age[1], data.storyType.Comedy);
            const stContext = $(sel.story4Name + data.name[1]).isDisplayed();
        ...
        it('TC-196 Verify that story/s moral is present', function () {
            inputValues4andClick(data.name[1], data.gender.it, data.age[1], data.storyType.Comedy);
            const stMoral = $(sel.storyMoral).isDisplayed();
            expect(stMoral).toEqual(true);
        });

        it('TC-197 Verify that story/s moral text is correct for Comedy', function () {
            inputValues4andClick(data.name[1], data.gender.it, data.age[1], data.storyType.Comedy);
